...
//
// Decoding server-side rather than shipping an ABI to the browser keeps the page small and
// means the numbers on it cannot drift from the chain: there is no cached copy to go stale.
...
const ORACLE = [
  ...
  { type:'function', name:'getRate', inputs:[{type:'bytes32'},{type:'uint256'}], stateMutability:'view',
    outputs:[{type:'uint256'},{type:'uint64'}] },
  { type:'function', name:'quorum', inputs:[], outputs:[{type:'uint256'}], stateMutability:'view' },
  { type:'function', name:'allowedDeviationBps', inputs:[{type:'uint256'}], outputs:[{type:'uint256'}], stateMutability:'view' },
...
export default async function handler(req, res) {
  ...
  try {
    const [block, gasPrice, feed, quorum, devFresh, devCeiling, dev1h] = await Promise.all([
      ...
      client.readContract({ address: D.oracle, abi: ORACLE, functionName: 'allowedDeviationBps', args: [3600n] }),
    ])
    ...
    res.status(200).end(j({
      addresses: D, pair: PAIR,
      block: { number: block.number, timestamp: block.timestamp },
      gasPriceGwei: Number(gasPrice) / 1e9,
      oracle: {
        rate: feed.rate, observedAt: feed.observedAt, postedAt: feed.postedAt,
        ageSeconds: Number(block.timestamp) - Number(feed.observedAt),
        quorum, devFreshBps: devFresh, devCeilingBps: devCeiling, devAt1hBps: dev1h,
      },
      pool,
      asOf: new Date().toISOString(),
    }))
  } catch (e) {
    res.status(502).end(j({ error: 'read failed', detail: String(e?.shortMessage ?? e?.message ?? e) }))
  }
}
